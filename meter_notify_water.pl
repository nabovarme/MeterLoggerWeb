...
while (1) {
	$sth = $dbh->prepare(qq[SELECT `serial`, `info`, `min_amount`, `valve_status`, `sw_version`, `email_notification`, `sms_notification`, `close_notification_time`, `notification_state`, `notification_sent_at` FROM meters WHERE `enabled` AND `type` = 'water' AND (`email_notification` OR `sms_notification`)]);
	$sth->execute;
			...
			if (($d->{notification_state}) == 0) {		# send close warning notification if not sent before
				if ($volume_time_left < (($d->{close_notification_time} / 3600) || CLOSE_WARNING_TIME)) {	# 3 days
					...
					syslog('info', "close warning sent for serial #" . $d->{serial} 
						...
						. ", time now: " . $time_now 
						. ", time last: " . $time_last);
					$notification = 'Water ' . $d->{info} . ' closing in ' . sprintf("%.0f", $volume_time_left / 24) . ' days. (' . $d->{serial} . ') ' . 
				...
				if ($volume_left <= 0.0) {			# no volume left	DEBUG: we send it before becouse of rounding error in firmware
					...
					syslog('info', "close notice sent for serial #" . $d->{serial} 
						...
						. ", time now: " . $time_now 
						. ", time last: " . $time_last);
					$notification = 'Water ' . $d->{info} . ' closed. (' . $d->{serial} . ') ' . 
						'https://meterlogger.net/detail_volume_acc.epl?serial=' . $d->{serial};
					if ($d->{sms_notification}) {
						sms_send($d->{sms_notification}, $notification);
					}
					...
					syslog('info', "open notice sent for serial #" . $d->{serial} 
						...
						. ", time now: " . $time_now 
						. ", time last: " . $time_last);
					$notification = 'Water ' . $d->{info} . ' open. ' . sprintf("%.0f", $volume_time_left / 24) . ' days left. (' . $d->{serial} . ') ' . 
						'https://meterlogger.net/detail_volume_acc.epl?serial=' . $d->{serial};
					if ($d->{sms_notification}) {
						sms_send($d->{sms_notification}, $notification);
					}
